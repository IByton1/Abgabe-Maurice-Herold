...
uint32_t my_hash(const string &key) {
  ...
  static const uint32_t p = 4294967291; // largest prime number that fits in 32 bits
  static const uint32_t a[] = {0x2d2f, 0x5501, 0x6b21, 0x66d3}; // random coefficients
  uint32_t h = 0;
  for (char c : key) {
    h = (h * a[c % size(a)]) % p; // multiply by a random coefficient and take mod p
    h = (h + c) % p; // add the character code and take mod p
  }
  static const uint32_t t[] = {0, 2, 3, 0, 1}; // offsets for resolving collisions
  return (t[h % size(t)] + h) % size(table); // add offset and take mod size(table)
